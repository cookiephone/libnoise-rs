...
pub(crate) fn noise2d(perm: &PermutationTable, point: [f64; 2]) -> f64 {
    ...
    let dy = y - y0 as f64;
    let dxs = smoothstep_3(dx);
    let dys = smoothstep_3(dy);
    // hashed gradient indices
    let x0 = x0.rem_euclid(PERMUTATION_TABLE_SIZE);
    let y0 = y0.rem_euclid(PERMUTATION_TABLE_SIZE);
    let gi00 = unsafe { perm.hash2d(x0, y0) } % CORNERPOINT_GRADIENT_LUT_2D_SIZE;
    let gi01 = unsafe { perm.hash2d(x0, y0 + 1) } % CORNERPOINT_GRADIENT_LUT_2D_SIZE;
    let gi10 = unsafe { perm.hash2d(x0 + 1, y0) } % CORNERPOINT_GRADIENT_LUT_2D_SIZE;
    let gi11 = unsafe { perm.hash2d(x0 + 1, y0 + 1) } % CORNERPOINT_GRADIENT_LUT_2D_SIZE;
    // compute contributions
...
pub(crate) fn noise3d(perm: &PermutationTable, point: [f64; 3]) -> f64 {
    ...
    let dxs = smoothstep_3(dx);
    let dys = smoothstep_3(dy);
    let dzs = smoothstep_3(dz);
    // hashed gradient indices
    let x0 = x0.rem_euclid(PERMUTATION_TABLE_SIZE);
    let y0 = y0.rem_euclid(PERMUTATION_TABLE_SIZE);
    let z0 = z0.rem_euclid(PERMUTATION_TABLE_SIZE);
    let gi000 = unsafe { perm.hash3d(x0, y0, z0) } % CORNERPOINT_GRADIENT_LUT_3D_SIZE;
    let gi001 = unsafe { perm.hash3d(x0, y0, z0 + 1) } % CORNERPOINT_GRADIENT_LUT_3D_SIZE;
    let gi010 = unsafe { perm.hash3d(x0, y0 + 1, z0) } % CORNERPOINT_GRADIENT_LUT_3D_SIZE;
    let gi011 = unsafe { perm.hash3d(x0, y0 + 1, z0 + 1) } % CORNERPOINT_GRADIENT_LUT_3D_SIZE;
    let gi100 = unsafe { perm.hash3d(x0 + 1, y0, z0) } % CORNERPOINT_GRADIENT_LUT_3D_SIZE;
    let gi101 = unsafe { perm.hash3d(x0 + 1, y0, z0 + 1) } % CORNERPOINT_GRADIENT_LUT_3D_SIZE;
    let gi110 = unsafe { perm.hash3d(x0 + 1, y0 + 1, z0) } % CORNERPOINT_GRADIENT_LUT_3D_SIZE;
    let gi111 = unsafe { perm.hash3d(x0 + 1, y0 + 1, z0 + 1) } % CORNERPOINT_GRADIENT_LUT_3D_SIZE;
    // compute contributions
    ...
    lerp(yn0, yn1, dzs) * 0.6666666666666666
}
...
pub(crate) fn noise4d(perm: &PermutationTable, point: [f64; 4]) -> f64 {
    ...
    let dys = smoothstep_3(dy);
    let dzs = smoothstep_3(dz);
    let dws = smoothstep_3(dw);
    // hashed gradient indices
    let x0 = x0.rem_euclid(PERMUTATION_TABLE_SIZE);
    let y0 = y0.rem_euclid(PERMUTATION_TABLE_SIZE);
    let z0 = z0.rem_euclid(PERMUTATION_TABLE_SIZE);
    let w0 = w0.rem_euclid(PERMUTATION_TABLE_SIZE);
    let gi0000 = unsafe { perm.hash4d(x0, y0, z0, w0) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi0001 = unsafe { perm.hash4d(x0, y0, z0, w0 + 1) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi0010 = unsafe { perm.hash4d(x0, y0, z0 + 1, w0) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi0011 = unsafe { perm.hash4d(x0, y0, z0 + 1, w0 + 1) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi0100 = unsafe { perm.hash4d(x0, y0 + 1, z0, w0) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi0101 = unsafe { perm.hash4d(x0, y0 + 1, z0, w0 + 1) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi0110 = unsafe { perm.hash4d(x0, y0 + 1, z0 + 1, w0) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi0111 =
        unsafe { perm.hash4d(x0, y0 + 1, z0 + 1, w0 + 1) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi1000 = unsafe { perm.hash4d(x0 + 1, y0, z0, w0) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi1001 = unsafe { perm.hash4d(x0 + 1, y0, z0, w0 + 1) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi1010 = unsafe { perm.hash4d(x0 + 1, y0, z0 + 1, w0) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi1011 =
        unsafe { perm.hash4d(x0 + 1, y0, z0 + 1, w0 + 1) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi1100 = unsafe { perm.hash4d(x0 + 1, y0 + 1, z0, w0) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi1101 =
        unsafe { perm.hash4d(x0 + 1, y0 + 1, z0, w0 + 1) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi1110 =
        unsafe { perm.hash4d(x0 + 1, y0 + 1, z0 + 1, w0) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    let gi1111 =
        unsafe { perm.hash4d(x0 + 1, y0 + 1, z0 + 1, w0 + 1) } % CORNERPOINT_GRADIENT_LUT_4D_SIZE;
    // compute contributions
