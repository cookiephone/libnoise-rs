...
pub fn noise4d(seed: u64, x: f64, y: f64, z: f64, w: f64) -> f64 {
    ...
    let gi0 = unsafe { hash4d(seed, is, js, ks, ls) } % GRADIENT_LUT_4D_SIZE;
    let gi1 = unsafe { hash4d(seed, is + i1, js + j1, ks + k1, ls + l1) } % GRADIENT_LUT_4D_SIZE;
    let gi2 = unsafe { hash4d(seed, is + i2, js + j2, ks + k2, ls + l2) } % GRADIENT_LUT_4D_SIZE;
    let gi3 = unsafe { hash4d(seed, is + i3, js + j3, ks + k3, ls + l3) } % GRADIENT_LUT_4D_SIZE;
    let gi4 = unsafe { hash4d(seed, is + 1, js + 1, ks + 1, ls + 1) } % GRADIENT_LUT_4D_SIZE;
    // compute contributions, safe because gradient lookup table is known
